export default async function (output, _hist, ...args) {
  if (args.length) {
    ...
    window.open('https://github.com/yadla-kartik' + name);
  } else {
    let repositories;
    try {
      repositories = await fetch(
        "https://api.github.com/users/yadla-kartik/repos?type=owner&sort=created"
      ).then((r) => r.json());
    } catch (e) {
    ...
    output.innerHTML += `
Checkout more Awesome Projects on my <span data-color="cyan"><a href="https://github.com/yadla-kartik?tab=repositories">Github</a></span>\n
`;
  }
}
